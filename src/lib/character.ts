...
export async function updateSpecV1Character(id: number, field: keyof CharacterTable, value: any) {
  try {
...
export async function updateCharacterGreeting(id: number, index: number, value: string) {
  try {
    const rows = await db.character.update(id, {
